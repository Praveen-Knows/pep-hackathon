...
for (let i = 0; i < subjects.length; i++) {
  ...
  getData(`../data/${subjects[i]}.json`).then((data) => {
    // console.log(data);
    let idx = 0;
    let obj = {};
    while (idx < qes) {
    ...
    questionList = questionList.concat(questionnaire); //appending all the question in a single array
...
for (let idx = 0; idx < subjects.length; idx++) {
  ...
  if (subjects[idx] === "dbms") {
    h3.innerText = "Database Management System";
  } else if (subjects[idx] === "oop") {
    h3.innerText = "Object Oriented Programming";
  } else if (subjects[idx] === "cn") {
    h3.innerText = "Computer Networks";
  } else if (subjects[idx] === "os") {
    h3.innerText = "Operating System";
  } else {
    h3.innerText = "PseudoCode";
  }
